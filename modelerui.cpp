...
const char* ModelerUI::fileDialog(Fl_Native_File_Chooser::Type dialogType, const char* filter, const char* title) {
  //------------------------------------------------------------------------
...
{
	const char* szFileName = fileDialog(Fl_Native_File_Chooser::BROWSE_FILE,
										"Animation Script (*.ani)\t*.ani",
...
{
	const char* szFileName = fileDialog(Fl_Native_File_Chooser::BROWSE_SAVE_FILE,
										"Animation Script (*.ani)\t*.ani",
...
inline void ModelerUI::cb_saveImageFile_i(Fl_Menu_*, void*) 
{
	const char* szFilename = fileDialog(Fl_Native_File_Chooser::BROWSE_SAVE_FILE,
	...
	m_pwndModelerView->saveImage(szFilename, filetype == 0 ? ".png" : ".jpg", quality);
	m_pwndModelerView->redraw();
}

void ModelerUI::cb_saveImageFile(Fl_Menu_* o, void* v) 
{
	((ModelerUI*)(o->parent()->user_data()))->cb_saveImageFile_i(o,v);
}
...
inline void ModelerUI::cb_saveMovie_i(Fl_Menu_*, void*)
{
	...
	int fileType = m_nativeChooser->filter_value();	//0: PNG, 1: JPG
	ext = fileType == 0 ? ".png" : ".jpg";
	#ifdef WIN32
		char szExt[_MAX_EXT];
		_splitpath(m_strMovieFileName.c_str(), NULL, NULL, NULL, szExt);

		if (!stricmp(szExt, ".png") || !stricmp(szExt, ".jpg")) {
...
void ModelerUI::redrawModelerView()
{
	...
	_snprintf(szFrameNum, 128, "%04d", m_iMovieFrameNum++);
		...
		strFileName += ext;
		m_pwndModelerView->saveImage(strFileName.c_str(), ext.c_str(), quality);
	}
}
#ifdef WIN32
...
m_nativeChooser(NULL)
{
	...
	m_pmiSaveBitmapFile->callback((Fl_Callback*)cb_saveImageFile);
